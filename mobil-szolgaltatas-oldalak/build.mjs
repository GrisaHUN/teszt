...
for (const file of readdirSync(join(here, 'src')).filter((f) => !f.startsWith('_') && f.endsWith('.html')).sort()) {
  ...
  const chunks = p.HTML.split(/^\s*\{\{SPLIT\}\}\s*$/m);
  const pageIds = [];
  chunks.forEach((chunk, idx) => {
    ...
    for (const [, href] of code.matchAll(/\shref="([^"]+)"/g)) if (href !== ZOHO && href !== 'tel:+36203734991' && !href.startsWith(`#${rootId}-i-`)) fail(`váratlan link: ${href}`);
